with first_hep as
  (select distinct 
          ...
          --max(i.charttime) over (partition by icud.icustay_id order by i.charttime) as fin_heparin_infusion_time
          --COUNT(i.charttime) as num_hep_events
...
last_hep as
  (select distinct 
          icud.icustay_id,
          --first_value(i.charttime) over (partition by icud.icustay_id order by i.charttime) as heparin_infusion_time,
          --max(i.charttime) as fin_heparin_infusion_time --over (partition by icud.icustay_id order by i.charttime) as fin_heparin_infusion_time
          max(i.charttime) as fin_heparin_infusion_time --over (partition by icud.icustay_id order by i.charttime) as fin_heparin_infusion_time
          --COUNT(i.charttime) as num_hep_events
   from mimic2v26.icustay_detail icud
   join mimic2v26.ioevents i on icud.icustay_id=i.icustay_id and i.itemid=137
...
--select count(*) from heparin_final
...
, final_data as
  (select DISTINCT bp.*,
          ...
          months_between(bp.heparin_infusion_time,bp.hospital_admit_dt)*30*24 AS first_injection_interval ,
          months_between(bp.fin_heparin_infusion_time,bp.hospital_admit_dt)*30*24 AS last_injection_interval
